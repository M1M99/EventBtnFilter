let goods = [
    ...
    // Uc duyme. Duymelere event tedbiq edin. Her duyme goods massivindeki bir maqazin olacaq. Maqazine klik etdiyiniz zaman mallar maqazin uzre filter olunmalidir

    function showArr(goods = []){
        ...
        goods.forEach(element => {
            ...
            `
            document.getElementById('list').appendChild(myTag)
        });
    }

    let ModaDunyasi = document.getElementById('btn1')
    let ZarifModa = document.getElementById('btn2')
    let AliyevModaCenter = document.getElementById('btn3')

    function filterForStoreName(goods = [], storeName){
        return goods.filter((a) => a.store_name === storeName)
    }

    ModaDunyasi.addEventListener('click', (a) => {
        let modaDunyasi = filterForStoreName(goods,'Moda Dünyası')
        showArr(modaDunyasi)
    })

    ZarifModa.addEventListener('click', (a) => {
        let modaDunyasi = filterForStoreName(goods,'Zərif Moda')
        showArr(modaDunyasi)
    })

    AliyevModaCenter.addEventListener('click', (a) => {
        let modaDunyasi = filterForStoreName(goods,'Əliyev Moda Mərkəzi')
        showArr(modaDunyasi)
    })
